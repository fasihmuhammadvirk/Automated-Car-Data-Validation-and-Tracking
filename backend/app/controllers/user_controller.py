...
from middleware.utils import get_hashed_password, verify_password, create_access_token_user, decode_access_token
...
def create_user(user_info: dict):
    ...
    jwt_access = create_access_token_user([user_info['cnic'],user_info['number_plate']])
...
def login_user(user_info: dict):
    ...
    jwt_access = create_access_token_user([user_info['cnic'],db_car.number_plate])
    db.commit()
    return {'usertoken': jwt_access}
